...
namespace Курсовой_проект_Жилманов_В.Ю._3_Des
{
    public static class DES
    {
        ...
        public static void TripleDesIncode(ulong[] notCriptBlocks, out ulong[] criptBlocks)
        {
            criptBlocks = new ulong[notCriptBlocks.Length];

            if (KeyGenerator.cipherMode == "CBC" || KeyGenerator.cipherMode == "ECB")
            {
                for (int i = 0; i < notCriptBlocks.Length; i++)
                {
                    if (KeyGenerator.cipherMode == "CBC")
                        notCriptBlocks[i] ^= i > 0 ? criptBlocks[i - 1] : KeyGenerator.IV;

                    criptBlocks[i] = DES.Encode(notCriptBlocks[i], KeyGenerator.keys[0]);
                    criptBlocks[i] = DES.Decode(criptBlocks[i], KeyGenerator.keys[1]);
                    criptBlocks[i] = DES.Encode(criptBlocks[i], KeyGenerator.keys[2]);
                } 
            }

            if (KeyGenerator.cipherMode == "CFB")
            {
                ulong bufIV = KeyGenerator.IV;
                for (int i = 0; i < notCriptBlocks.Length; i++)
                {
                    bufIV = DES.Encode(bufIV, KeyGenerator.keys[0]);
                    bufIV = DES.Decode(bufIV, KeyGenerator.keys[1]);
                    bufIV = DES.Encode(bufIV, KeyGenerator.keys[2]);

                    bufIV ^= notCriptBlocks[i];
                    criptBlocks[i] = bufIV;
                }
            }

            if (KeyGenerator.cipherMode == "OFB")
            {
                ulong bufIV = KeyGenerator.IV;
                for (int i = 0; i < notCriptBlocks.Length; i++)
                {
                    ...
                    criptBlocks[i] = bufIV ^ notCriptBlocks[i];
                }
            }

        }

        public static void TripleDesDecode(ulong[] criptBlocks, out ulong[] DeCriptBlocks)
        {
            ...
            if (KeyGenerator.cipherMode == "CBC" || KeyGenerator.cipherMode == "ECB")
            {
                ...
                {
                    DeCriptBlocks[i] = DES.Decode(criptBlocks[i], KeyGenerator.keys[2]);
                    DeCriptBlocks[i] = DES.Encode(DeCriptBlocks[i], KeyGenerator.keys[1]);
                    DeCriptBlocks[i] = DES.Decode(DeCriptBlocks[i], KeyGenerator.keys[0]);

                    if (KeyGenerator.cipherMode == "CBC")
                        DeCriptBlocks[i] ^= i > 0 ? criptBlocks[i - 1] : KeyGenerator.IV;
                }
            }
            ...
            if(KeyGenerator.cipherMode == "CFB")
            {
                ...
                for (int i = 0; i < criptBlocks.Length; i++)
                {
                    ...
                    DeCriptBlocks[i] = bufIV ^ criptBlocks[i];
                    bufIV = criptBlocks[i];
                }
            }
        ...
        private static ulong AddBitsInKey(ulong key)
        {
            ulong mask = 1;
            int controlEven = 0;
            for (int i = 0; i < 7; i++)
        ...
        public static List<ulong> KeyCreating(ulong key)
        {
        ...
        {
            key = AddBitsInKey(key);

            ulong ip = Permute(message, IP);
            var keyBlanking = KeyCreating(key);
            ...
            for (int i = -1; i < 16; i++)
            {
                ...
                {
                    Left = pair.Right,
                    Right = pair.Left ^ F(pair.Right, keyBlanking[i + 1])
                };
            }
        ...
        public static ulong Decode(ulong message, ulong key)
        {
            key = AddBitsInKey(key);
